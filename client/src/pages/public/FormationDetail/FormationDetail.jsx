...
import { ArrowLeft, Calendar, MapPin, Users, User, Award, CheckCircle2, Clock, Coins } from 'lucide-react';
...
export default function FormationDetail() {
  ...
  useEffect(() => {
    const fetchFormation = async () => {
      try {
        ...
        console.log('🔍 Recherche formation avec ID:', id);
  ...
  // ✅ Vérifier si l'utilisateur peut s'inscrire (uniquement pour les formations à venir)
  const canEnroll = () => {
    if (!formation) return false;
    // ✅ Le bouton s'affiche UNIQUEMENT si le statut est "upcoming" (À venir)
    return formation.status === 'upcoming';
  };

  // ✅ Vérifier si l'utilisateur est déjà inscrit (à implémenter avec une API dédiée si besoin)
  ...
  const handleEnroll = async () => {
    ...
    if (!canEnroll()) {
      toast.error('Les inscriptions pour cette formation ne sont pas ouvertes');
      return;
    }
    ...
    try {
      ...
      const enrollmentData = {
        ...
        notes: `Inscription automatique depuis la page publique le ${new Date().toLocaleDateString('fr-FR')}`
      };
      ...
      if (response?.status === 'success' || response?.data) {
        toast.success(
          `Votre inscription à "${formation.title}" a été enregistrée avec succès !\n\n` +
          `Un administrateur validera votre inscription dans les plus brefs délais.\n\n` +
          `Vous pouvez suivre l'état de votre inscription dans votre espace personnel.`,
          { duration: 8000 }
        );
        
        // ✅ Redirection vers la page des inscriptions admin
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
      <Link 
        to="/formations" 
        className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-primary-600 dark:text-slate-400 dark:hover:text-primary-400 mb-6 font-medium transition-colors"
      >
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 shadow-card space-y-4 sticky top-24">
            ...
            {/* ✅ Bouton d'inscription - UNIQUEMENT pour les formations "À venir" (status = upcoming) */}
            {formation.status === 'upcoming' && !isFull && (
              <button 
                onClick={handleEnroll}
                disabled={enrolling || isAlreadyEnrolled}
                className="w-full btn-primary py-2.5 mt-2 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {enrolling ? (
                  <>
            ...
            {formation.status !== 'upcoming' && formation.status !== 'completed' && formation.status !== 'cancelled' && (
